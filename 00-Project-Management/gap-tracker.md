...
| Date | Update |
|---|---|
| 2026-04-13 | **WCAG 2.1 AA accessibility work completed through iterative review.** `eslint-plugin-jsx-a11y` added to lint pipeline. Full WAI-ARIA patterns implemented across all interactive components: EmbedModal (dialog focus trap, focus restore, close button label), FilterMultiSelect (aria-activedescendant, option ids, listbox aria-labelledby, group roles, search input label), FilterSidebar (filter aside landmark, collapse button aria-expanded, download menu ARIA pattern with keyboard nav), DownloadButton (menu id/aria-controls, icon-only aria-label, single-option keyboard activation, tab roles in EmbedModal), FullscreenChart (filter aside landmark, collapse button labels). Browser support declared via `browserslist`: Chrome, Edge, Firefox (incl. ESR), Safari, iOS Safari — last 2 versions each. Responsive testing extended to 375px (iPhone SE). Both Phase 4 open questions closed. |
